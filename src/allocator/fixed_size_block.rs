...
impl NewFixedBlockAllocator {
    ...
    pub const fn new() -> Self {
        ...
        Self {inner: UnsafeCell::new(
            FixedBlockInner {
                list_heads: [const { super::allocator_linked_list::RawLinkedList::new() };BLOCK_SIZES.len()],
    ...
    fn extend(&self) {
        ...
        let ptr = HeapAlloc::virt_allocate(&alloc, layout)
            .expect("System encountered a memory error");

        let region_start = VirtAddr::from_ptr(
            ptr
                .cast::<u8>().as_ptr()
        );
        ...
        unsafe {
            ...
            mem::mem_map::map_page(x86_64::structures::paging::Page::containing_address(region_start), flags);
...
impl FixedBlockInner {
    ...
    fn crack(&mut self, target: usize) -> Result<(),()> {
        ...
        if t_large == 0 { // t_large is only 0 if no free nodes are found
            return Err(());
        }
...
unsafe impl Allocator for NewFixedBlockAllocator {
    ...
    fn allocate(&self, layout: Layout) -> Result<NonNull<[u8]>, AllocError> {
        ...
        let slice = unsafe { core::slice::from_raw_parts_mut::<u8>(alloc.list_heads[index].fetch().unwrap(), BLOCK_SIZES[index]) };
...
impl InferiorAllocator for NewFixedBlockAllocator {
    ...
    unsafe fn init(&self, ptr: *mut [u8; Self::INIT_BYTES]) {
        ...
        for i in 0..count {
            ...
            alloc.list_heads.last_mut().unwrap().add(location as *mut u64)
    ...
    unsafe fn force_dealloc(&self, ptr: NonNull<u8>, layout: Layout) {
        ...
        assert_eq!((addr as *const u64).align_offset(8),0,"FixedBlockAllocator cannot handle regions smaller than 8 bytes");

        for i in BLOCK_SIZES.iter().rev() { // biggest first
            let num = *i;
