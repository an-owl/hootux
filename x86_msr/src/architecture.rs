...
//! all MSR names in this module the their prefix "ia-32_" omitted and written in camel case
//! i.e "IA32_TIME_STAMP_COUNTER" becomes "TimeStampCounter"
//!
//! MSR addresses can be found in their
//!
...
impl MsrFlags for SpecCtrlFlags{
    ...
    fn new(high: u32, low: u32) -> Self {
        Self{bits: Self::bitfield(high,low)}
    }
}
...
impl Msr<ApicBaseData> for ApicBase{
    ...
    fn availability() -> MsrAvailability {
        ...
        let feature = cpuid.get_feature_info().expect("unable to get feature info");
        ...
        if ((feature.family_id() == 6) && (feature.model_id() >= 1)) || feature.family_id() > 6{
            ...
            if((feature.family_id() == 6) && (feature.model_id() >= 0x1a)) || feature.family_id() > 6 {
